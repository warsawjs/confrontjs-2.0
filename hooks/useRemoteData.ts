...
export const fetchData = (filename: string) => fetch(`${url}/${filename}.json`).then(r => r.json()).then(r => {
  if (filename === 'info' || filename === 'venue') {
    const info = Object.assign({}, ...r.map((e: any) => ({[e.id]: e.data})))
    return info;
  }
  return (!r || r.push) ? r || [] : Object.values(r);
})

export function useRemoteData(filename: string, setData: boolean | ((s: object[]) => void) = false, interval = 2) {
...
export function useData(initialData: {speakers?: any, [key: string]: any}, sheetName?: string, intervalInSec?: number) {
  const [data, setData] = React.useState(initialData);
  Object.keys(initialData).map(sheetName => {
    useRemoteData(sheetName, (data) => setData((d: any) => {
      const filteredData = data.filter ? data.filter((e: any) => Object.values(e).some(Boolean)) : data;
